"use client";
import { RemoveTeacherConnection } from "@/action/connection";
import {
  AlertDialog,
  AlertDialogCancel,
  AlertDialogContent,
...
import { LessonWithTeacher } from "./my-teacher-container";
...
const MyTeacherCard = ({ data }: Props) => {
  const [open, setOpen] = useState(false);
  const [pending, startTransition] = useTransition();

  const onRemove = () => {
    startTransition(() => {
      RemoveTeacherConnection(data.id).then((res) => {
        if (!res.success) {
